...
(function () {
    ...
    function makeDove() {
        ...
        body.setAttribute('d', 'M6 24 Q18 14 34 18 Q46 20 56 14 Q54 22 44 26 Q34 30 22 28 Q12 28 6 24 Z');
        body.setAttribute('fill', '#ffffff');
        body.setAttribute('stroke', 'rgba(107,77,140,0.45)');
        body.setAttribute('stroke-width', '0.5');

        const head = document.createElementNS(SVG_NS, 'circle');
        head.setAttribute('cx', '54'); head.setAttribute('cy', '14'); head.setAttribute('r', '4');
        head.setAttribute('fill', '#ffffff');
        head.setAttribute('stroke', 'rgba(107,77,140,0.45)');
        head.setAttribute('stroke-width', '0.5');

        const beak = document.createElementNS(SVG_NS, 'path');
        beak.setAttribute('d', 'M58 14 L62 13 L58 16 Z');
        beak.setAttribute('fill', '#C18EDA');

        const wingBack = document.createElementNS(SVG_NS, 'path');
        wingBack.setAttribute('class', 'wing wing-back');
        wingBack.setAttribute('d', 'M22 20 Q26 4 40 14 Q34 18 22 20 Z');
        wingBack.setAttribute('fill', '#f3eaf6');
        wingBack.setAttribute('stroke', 'rgba(107,77,140,0.3)');
        wingBack.setAttribute('stroke-width', '0.4');

        const wing = document.createElementNS(SVG_NS, 'path');
        wing.setAttribute('class', 'wing');
        wing.setAttribute('d', 'M22 22 Q28 6 42 16 Q34 24 22 22 Z');
        wing.setAttribute('fill', '#ffffff');
        wing.setAttribute('stroke', 'rgba(107,77,140,0.45)');
        wing.setAttribute('stroke-width', '0.5');

        svg.appendChild(body);
        svg.appendChild(wingBack);
    ...
    function launch() {
        ...
        const startY = window.innerHeight * (0.05 + Math.random() * 0.7);
        const endY = startY + (Math.random() * 200 - 100);
        const duration = 12000 + Math.random() * 9000;

        d.style.top = startY + 'px';
        d.style.transform = `translateX(${goLeft ? window.innerWidth + 80 : -100}px) scale(${scale}) ${goLeft ? 'scaleX(-1)' : ''}`;
        d.style.transition = `transform ${duration}ms linear, opacity 1.5s ease`;
        d.style.opacity = '0';
        ...
        requestAnimationFrame(() => {
            ...
            const endX = goLeft ? -120 : window.innerWidth + 80;
            const dy = endY - startY;
            d.style.transform = `translate(${endX}px, ${dy}px) scale(${scale}) ${goLeft ? 'scaleX(-1)' : ''}`;
        });
